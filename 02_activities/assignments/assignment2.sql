...
-- COALESCE
/* 1. Fix NULL values in product details using COALESCE */
SELECT 
...
/* 1. Number each customer's visits to the farmer's market */
SELECT 
    customer_id, 
    market_date, 
...
/* 2. Reverse numbering for customer visits and filter most recent */
...
/* 1. Extract product descriptions using SUBSTR and INSTR */
SELECT 
    product_name, 
...
/* 2. Filter product_size values that contain a number */
SELECT 
    product_size
FROM product
...
/* 1. Calculate potential revenue for vendors per product */
WITH ProductCustomers AS (
    SELECT 
        vi.vendor_id, 
        vi.product_id, 
        vi.original_price * 5 * (SELECT COUNT(*) FROM customer) AS potential_revenue
...
/* 1. Create product_units table with CURRENT_TIMESTAMP */
...
VALUES (999, 'Sample Product', 'Large', 'unit', CURRENT_TIMESTAMP);
...
/* 1. Add current_quantity to product_units and update it */

-- Update current_quantity with the last quantity value from vendor_inventory
WITH LatestQuantity AS (
    SELECT 
        product_id, 
...
SET current_quantity = (
    ...
    FROM LatestQuantity
    WHERE LatestQuantity.product_id = product_units.product_id
)
WHERE EXISTS (
    SELECT 1
    FROM LatestQuantity
